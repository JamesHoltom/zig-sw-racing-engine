...
const GlyphInfo = raylib.GlyphInfo;
const Image = raylib.Image;
const Rectangle = raylib.Rectangle;
const Texture2D = raylib.Texture2D;
const Vector2 = raylib.Vector2;
const Vector3 = raylib.Vector3;
const CheckCollisionPointRec = raylib.CheckCollisionPointRec;
const CodepointToUTF8 = raylib.CodepointToUTF8;
const DecompressData = raylib.DecompressData;
const DrawRectangle = raylib.DrawRectangle;
const DrawRectangleGradientEx = raylib.DrawRectangleGradientEx;
const DrawRectangleGradientV = raylib.DrawRectangleGradientV;
const DrawTextCodepoint = raylib.DrawTextCodepoint;
const Fade = raylib.Fade;
const GetCharPressed = raylib.GetCharPressed;
const GetColor = raylib.GetColor;
const GetCodepoint = raylib.GetCodepoint;
const GetCodepointNext = raylib.GetCodepointNext;
const GetCodepointPrevious = raylib.GetCodepointPrevious;
const GetDirectoryPath = raylib.GetDirectoryPath;
const GetFontDefault = raylib.GetFontDefault;
const GetGlyphIndex = raylib.GetGlyphIndex;
const GetMousePosition = raylib.GetMousePosition;
const GetMouseWheelMove = raylib.GetMouseWheelMove;
const GetScreenToWorldRay = raylib.GetScreenToWorldRay;
const GetScreenWidth = raylib.GetScreenWidth;
const IsKeyDown = raylib.IsKeyDown;
const IsKeyPressed = raylib.IsKeyPressed;
const IsMouseButtonDown = raylib.IsMouseButtonDown;
const IsMouseButtonPressed = raylib.IsMouseButtonPressed;
const IsMouseButtonReleased = raylib.IsMouseButtonReleased;
const LoadCodepoints = raylib.LoadCodepoints;
const LoadFileText = raylib.LoadFileText;
const LoadFontEx = raylib.LoadFontEx;
const LoadTextureFromImage = raylib.LoadTextureFromImage;
const MeasureTextEx = raylib.MeasureTextEx;
const SetShapesTexture = raylib.SetShapesTexture;
const TextFormat = raylib.TextFormat;
const TextToFloat = raylib.TextToFloat;
const TextToInteger = raylib.TextToInteger;
const UnloadCodepoints = raylib.UnloadCodepoints;
const UnloadFileText = raylib.UnloadFileText;
const UnloadTexture = raylib.UnloadTexture;
const MATERIAL_MAP_ALBEDO = raylib.MATERIAL_MAP_ALBEDO;
const MATERIAL_MAP_METALNESS = raylib.MATERIAL_MAP_METALNESS;
const MOUSE_BUTTON_LEFT = raylib.MOUSE_BUTTON_LEFT;
const MOUSE_BUTTON_MIDDLE = raylib.MOUSE_BUTTON_MIDDLE;
const MOUSE_BUTTON_RIGHT = raylib.MOUSE_BUTTON_RIGHT;
const KEY_BACKSPACE = raylib.KEY_BACKSPACE;
const KEY_DELETE = raylib.KEY_DELETE;
const KEY_END = raylib.KEY_END;
const KEY_ENTER = raylib.KEY_ENTER;
const KEY_HOME = raylib.KEY_HOME;
const KEY_LEFT_CONTROL = raylib.KEY_LEFT_CONTROL;
const KEY_LEFT_SHIFT = raylib.KEY_LEFT_SHIFT;
const KEY_DOWN = raylib.KEY_DOWN;
const KEY_LEFT = raylib.KEY_LEFT;
const KEY_RIGHT = raylib.KEY_RIGHT;
const KEY_UP = raylib.KEY_UP;
const SHADER_LOC_MAP_ALBEDO = raylib.SHADER_LOC_MAP_ALBEDO;
const SHADER_LOC_MAP_METALNESS = raylib.SHADER_LOC_MAP_METALNESS;
...
pub const struct_GuiStyleProp = extern struct {
    controlId: c_ushort = std.mem.zeroes(c_ushort),
    propertyId: c_ushort = std.mem.zeroes(c_ushort),
    propertyValue: c_int = std.mem.zeroes(c_int),
};
                        ...
                        @as(c_int, 102) => {
                            {
                                ...
                                var font: Font = Font{
                                    ...
                                    .glyphCount = 0,
                                    .glyphPadding = 0,
                                    .texture = std.mem.zeroes(Texture2D),
                                    .recs = null,
                                    .glyphs = null,
                                };
                                _ = &font;
                ...
                while (k < @as(c_int, 32)) : (k += 1) {
                    if (((blk: {
                        ...
                        GuiDrawRectangle(Rectangle{
                            .x = @as(f32, @floatFromInt(posX)) + @as(f32, @floatFromInt(std.zig.c_translation.signedRemainder(k, @as(c_int, 16)) * pixelSize)),
    ...
    if (((((state != @as(c_uint, @bitCast(STATE_DISABLED))) and !(GuiGetStyle(TEXTBOX, TEXT_READONLY) != 0)) and !guiLocked) and !guiControlExclusiveMode) and (wrapMode == TEXT_WRAP_NONE)) {
        ...
        if (editMode) {
            ...
            if ((textLength > textBoxCursorIndex) and ((@as(c_int, @intFromBool(IsKeyPressed(KEY_DELETE))) != 0) or ((@as(c_int, @intFromBool(IsKeyDown(KEY_DELETE))) != 0) and (autoCursorCooldownCounter >= @as(c_int, 40))))) {
                ...
                if ((@as(c_int, @intFromBool(IsKeyPressed(KEY_DELETE))) != 0) or (std.zig.c_translation.signedRemainder(autoCursorDelayCounter, @as(c_int, 1)) == @as(c_int, 0))) {
                    var nextCodepointSize: c_int = 0;
                    _ = &nextCodepointSize;
            ...
            if ((textLength > @as(c_int, 0)) and ((@as(c_int, @intFromBool(IsKeyPressed(KEY_BACKSPACE))) != 0) or ((@as(c_int, @intFromBool(IsKeyDown(KEY_BACKSPACE))) != 0) and (autoCursorCooldownCounter >= @as(c_int, 40))))) {
                autoCursorDelayCounter += 1;
                if ((@as(c_int, @intFromBool(IsKeyPressed(KEY_BACKSPACE))) != 0) or (std.zig.c_translation.signedRemainder(autoCursorDelayCounter, @as(c_int, 1)) == @as(c_int, 0))) {
            ...
            if ((@as(c_int, @intFromBool(IsKeyPressed(KEY_LEFT))) != 0) or ((@as(c_int, @intFromBool(IsKeyDown(KEY_LEFT))) != 0) and (autoCursorCooldownCounter > @as(c_int, 40)))) {
                autoCursorDelayCounter += 1;
                if ((@as(c_int, @intFromBool(IsKeyPressed(KEY_LEFT))) != 0) or (std.zig.c_translation.signedRemainder(autoCursorDelayCounter, @as(c_int, 1)) == @as(c_int, 0))) {
                    var prevCodepointSize: c_int = 0;
                    _ = &prevCodepointSize;
                    _ = GetCodepointPrevious(text + @as(usize, @bitCast(@as(isize, @intCast(textBoxCursorIndex)))), &prevCodepointSize);
                ...
                autoCursorDelayCounter += 1;
                if ((@as(c_int, @intFromBool(IsKeyPressed(KEY_RIGHT))) != 0) or (std.zig.c_translation.signedRemainder(autoCursorDelayCounter, @as(c_int, 1)) == @as(c_int, 0))) {
                    var nextCodepointSize: c_int = 0;
                    _ = &nextCodepointSize;
                    _ = GetCodepointNext(text + @as(usize, @bitCast(@as(isize, @intCast(textBoxCursorIndex)))), &nextCodepointSize);
    ...
    if (subdivs > @as(c_int, 0)) {
        {
            ...
            while (i < linesV) : (i += 1) {
                ...
                GuiDrawRectangle(lineV, @as(c_int, 0), Color{
                    ...
                    .b = @as(u8, @bitCast(@as(i8, @truncate(@as(c_int, 0))))),
                    .a = @as(u8, @bitCast(@as(i8, @truncate(@as(c_int, 0))))),
                }, if (std.zig.c_translation.signedRemainder(i, subdivs) == @as(c_int, 0)) GuiFade(GetColor(@as(c_uint, @bitCast(color))), 0.15000000596046448 * @as(f32, @floatFromInt(@as(c_int, 4)))) else GuiFade(GetColor(@as(c_uint, @bitCast(color))), 0.15000000596046448));
            }
        }
        {
            var i: c_int = 0;
            ...
            while (x < checksX) : (x += 1) {
                {
                    ...
                    while (y < checksY) : (y += 1) {
                        ...
                        GuiDrawRectangle(check, @as(c_int, 0), Color{
                            ...
                            .b = @as(u8, @bitCast(@as(i8, @truncate(@as(c_int, 0))))),
                            .a = @as(u8, @bitCast(@as(i8, @truncate(@as(c_int, 0))))),
                        }, if (std.zig.c_translation.signedRemainder(x + y, @as(c_int, 2)) != 0) Fade(GetColor(@as(c_uint, @bitCast(GuiGetStyle(COLORPICKER, BORDER_COLOR_DISABLED)))), 0.4000000059604645) else Fade(GetColor(@as(c_uint, @bitCast(GuiGetStyle(COLORPICKER, BASE_COLOR_DISABLED)))), 0.4000000059604645));
                    }
                }
            }
        }
...
pub const __mbstate_t = extern struct {
    ...
    __value: union_unnamed_2 = std.mem.zeroes(union_unnamed_2),
};
...
pub const struct_timeval = extern struct {
    tv_sec: __time_t = std.mem.zeroes(__time_t),
    tv_usec: __suseconds_t = std.mem.zeroes(__suseconds_t),
};
pub const struct_timespec = extern struct {
    tv_sec: __time_t = std.mem.zeroes(__time_t),
    tv_nsec: __syscall_slong_t = std.mem.zeroes(__syscall_slong_t),
};
...
const struct_unnamed_3 = extern struct {
    __low: c_uint = std.mem.zeroes(c_uint),
    __high: c_uint = std.mem.zeroes(c_uint),
};
...
pub const struct___pthread_internal_list = extern struct {
    __prev: [*c]struct___pthread_internal_list = std.mem.zeroes([*c]struct___pthread_internal_list),
    __next: [*c]struct___pthread_internal_list = std.mem.zeroes([*c]struct___pthread_internal_list),
};
pub const __pthread_list_t = struct___pthread_internal_list;
pub const struct___pthread_internal_slist = extern struct {
    __next: [*c]struct___pthread_internal_slist = std.mem.zeroes([*c]struct___pthread_internal_slist),
};
pub const __pthread_slist_t = struct___pthread_internal_slist;
pub const struct___pthread_mutex_s = extern struct {
    __lock: c_int = std.mem.zeroes(c_int),
    __count: c_uint = std.mem.zeroes(c_uint),
    __owner: c_int = std.mem.zeroes(c_int),
    __nusers: c_uint = std.mem.zeroes(c_uint),
    __kind: c_int = std.mem.zeroes(c_int),
    __spins: c_short = std.mem.zeroes(c_short),
    __elision: c_short = std.mem.zeroes(c_short),
    __list: __pthread_list_t = std.mem.zeroes(__pthread_list_t),
};
pub const struct___pthread_rwlock_arch_t = extern struct {
    __readers: c_uint = std.mem.zeroes(c_uint),
    __writers: c_uint = std.mem.zeroes(c_uint),
    __wrphase_futex: c_uint = std.mem.zeroes(c_uint),
    __writers_futex: c_uint = std.mem.zeroes(c_uint),
    __pad3: c_uint = std.mem.zeroes(c_uint),
    __pad4: c_uint = std.mem.zeroes(c_uint),
    __cur_writer: c_int = std.mem.zeroes(c_int),
    __shared: c_int = std.mem.zeroes(c_int),
    __rwelision: i8 = std.mem.zeroes(i8),
    __pad1: [7]u8 = std.mem.zeroes([7]u8),
    __pad2: c_ulong = std.mem.zeroes(c_ulong),
    __flags: c_uint = std.mem.zeroes(c_uint),
};
pub const struct___pthread_cond_s = extern struct {
    ...
    __g_refs: [2]c_uint = std.mem.zeroes([2]c_uint),
    __g_size: [2]c_uint = std.mem.zeroes([2]c_uint),
    __g1_orig_size: c_uint = std.mem.zeroes(c_uint),
    __wrefs: c_uint = std.mem.zeroes(c_uint),
    __g_signals: [2]c_uint = std.mem.zeroes([2]c_uint),
};
pub const __tss_t = c_uint;
pub const __thrd_t = c_ulong;
pub const __once_flag = extern struct {
    __data: c_int = std.mem.zeroes(c_int),
};
...
pub const struct_random_data = extern struct {
    fptr: [*c]i32 = std.mem.zeroes([*c]i32),
    rptr: [*c]i32 = std.mem.zeroes([*c]i32),
    state: [*c]i32 = std.mem.zeroes([*c]i32),
    rand_type: c_int = std.mem.zeroes(c_int),
    rand_deg: c_int = std.mem.zeroes(c_int),
    rand_sep: c_int = std.mem.zeroes(c_int),
    end_ptr: [*c]i32 = std.mem.zeroes([*c]i32),
};
...
pub const struct_drand48_data = extern struct {
    ...
    __c: c_ushort = std.mem.zeroes(c_ushort),
    __init: c_ushort = std.mem.zeroes(c_ushort),
    __a: c_ulonglong = std.mem.zeroes(c_ulonglong),
};
...
pub const struct___locale_struct = extern struct {
    __locales: [13]?*struct___locale_data_4 = std.mem.zeroes([13]?*struct___locale_data_4),
        ...
        if (fontDataSize > @as(c_int, 0)) {
            ...
            if (font.texture.id != @as(c_uint, @bitCast(@as(c_int, 0)))) {
                ...
                if (@as(c_int, @bitCast(@as(c_int, version))) >= @as(c_int, 400)) {
                    ...
                    fileDataPtr += @as(usize, @intCast(@sizeOf(c_int)));
                }
                if ((recsDataCompressedSize > @as(c_int, 0)) and (recsDataCompressedSize != recsDataSize)) {
                    ...
                    font.recs = @as([*c]Rectangle, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, font.glyphCount))), @sizeOf(Rectangle)))));
                    {
                        ...
                        while (i < font.glyphCount) : (i += 1) {
                            ...
                            fileDataPtr += @as(usize, @intCast(@sizeOf(Rectangle)));
                        }
                    }
                }
        ...
        while (i < lineCount) : (i += 1) {
            ...
            while (true) {
                switch (alignmentVertical) {
                    ...
                    @as(c_int, 1) => {
                        textBoundsPosition.y = (((textBounds.y + posOffsetY) + (textBounds.height / @as(f32, @floatFromInt(@as(c_int, 2))))) - (totalHeight / @as(f32, @floatFromInt(@as(c_int, 2))))) + @as(f32, @floatFromInt(std.zig.c_translation.signedRemainder(@as(c_int, @intFromFloat(textBounds.height)), @as(c_int, 2))));
                        break;
                    },
                    @as(c_int, 2) => {
                        textBoundsPosition.y = (((textBounds.y + posOffsetY) + textBounds.height) - totalHeight) + @as(f32, @floatFromInt(std.zig.c_translation.signedRemainder(@as(c_int, @intFromFloat(textBounds.height)), @as(c_int, 2))));
                        break;
                    },
                    else => break,
                }
            ...
            _ = &ellipsisWidth;
            var overflowReached: bool = @as(c_int, 0) != 0;
            _ = &overflowReached;
            {
                var c: c_int = 0;
                _ = &c;
...
pub const va_start = @compileError("unable to translate macro: undefined identifier `__builtin_va_start`"); // /home/jholtom/Programming/Zig/zig-linux-x86_64-0.12.0/lib/include/stdarg.h:33:9
pub const va_end = @compileError("unable to translate macro: undefined identifier `__builtin_va_end`"); // /home/jholtom/Programming/Zig/zig-linux-x86_64-0.12.0/lib/include/stdarg.h:35:9
pub const va_arg = @compileError("unable to translate C expr: unexpected token 'an identifier'"); // /home/jholtom/Programming/Zig/zig-linux-x86_64-0.12.0/lib/include/stdarg.h:36:9
pub const __va_copy = @compileError("unable to translate macro: undefined identifier `__builtin_va_copy`"); // /home/jholtom/Programming/Zig/zig-linux-x86_64-0.12.0/lib/include/stdarg.h:41:9
pub const va_copy = @compileError("unable to translate macro: undefined identifier `__builtin_va_copy`"); // /home/jholtom/Programming/Zig/zig-linux-x86_64-0.12.0/lib/include/stdarg.h:46:9
pub const RAYGUI_LOG = @compileError("unable to translate C expr: expected ')' instead got '...'"); // /home/jholtom/Programming/zig-sw-racing-engine/libs/raygui/src/raygui.h:361:11
...
pub const DEG2RAD = std.zig.c_translation.MacroArithmetic.div(PI, @as(f32, 180.0));
pub const RAD2DEG = std.zig.c_translation.MacroArithmetic.div(@as(f32, 180.0), PI);
...
pub const LIGHTGRAY = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 200), @as(c_int, 200), @as(c_int, 200), @as(c_int, 255) });
pub const GRAY = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 130), @as(c_int, 130), @as(c_int, 130), @as(c_int, 255) });
pub const DARKGRAY = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 80), @as(c_int, 80), @as(c_int, 80), @as(c_int, 255) });
pub const YELLOW = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 253), @as(c_int, 249), @as(c_int, 0), @as(c_int, 255) });
pub const GOLD = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 255), @as(c_int, 203), @as(c_int, 0), @as(c_int, 255) });
pub const ORANGE = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 255), @as(c_int, 161), @as(c_int, 0), @as(c_int, 255) });
pub const PINK = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 255), @as(c_int, 109), @as(c_int, 194), @as(c_int, 255) });
pub const RED = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 230), @as(c_int, 41), @as(c_int, 55), @as(c_int, 255) });
pub const MAROON = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 190), @as(c_int, 33), @as(c_int, 55), @as(c_int, 255) });
pub const GREEN = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 0), @as(c_int, 228), @as(c_int, 48), @as(c_int, 255) });
pub const LIME = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 0), @as(c_int, 158), @as(c_int, 47), @as(c_int, 255) });
pub const DARKGREEN = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 0), @as(c_int, 117), @as(c_int, 44), @as(c_int, 255) });
pub const SKYBLUE = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 102), @as(c_int, 191), @as(c_int, 255), @as(c_int, 255) });
pub const BLUE = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 0), @as(c_int, 121), @as(c_int, 241), @as(c_int, 255) });
pub const DARKBLUE = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 0), @as(c_int, 82), @as(c_int, 172), @as(c_int, 255) });
pub const PURPLE = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 200), @as(c_int, 122), @as(c_int, 255), @as(c_int, 255) });
pub const VIOLET = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 135), @as(c_int, 60), @as(c_int, 190), @as(c_int, 255) });
pub const DARKPURPLE = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 112), @as(c_int, 31), @as(c_int, 126), @as(c_int, 255) });
pub const BEIGE = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 211), @as(c_int, 176), @as(c_int, 131), @as(c_int, 255) });
pub const BROWN = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 127), @as(c_int, 106), @as(c_int, 79), @as(c_int, 255) });
pub const DARKBROWN = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 76), @as(c_int, 63), @as(c_int, 47), @as(c_int, 255) });
pub const WHITE = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 255), @as(c_int, 255), @as(c_int, 255), @as(c_int, 255) });
pub const BLACK = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 0), @as(c_int, 0), @as(c_int, 0), @as(c_int, 255) });
pub const BLANK = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 0), @as(c_int, 0), @as(c_int, 0), @as(c_int, 0) });
pub const MAGENTA = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 255), @as(c_int, 0), @as(c_int, 255), @as(c_int, 255) });
pub const RAYWHITE = std.mem.zeroInit(CLITERAL(Color), .{ @as(c_int, 245), @as(c_int, 245), @as(c_int, 245), @as(c_int, 255) });
...
pub const WNOWAIT = std.zig.c_translation.promoteIntLiteral(c_int, 0x01000000, .hex);
pub const __WNOTHREAD = std.zig.c_translation.promoteIntLiteral(c_int, 0x20000000, .hex);
...
pub inline fn __WEXITSTATUS(status: anytype) @TypeOf((status & std.zig.c_translation.promoteIntLiteral(c_int, 0xff00, .hex)) >> @as(c_int, 8)) {
    _ = &status;
    return (status & std.zig.c_translation.promoteIntLiteral(c_int, 0xff00, .hex)) >> @as(c_int, 8);
}
...
pub inline fn __WIFSIGNALED(status: anytype) @TypeOf((std.zig.c_translation.cast(i8, (status & @as(c_int, 0x7f)) + @as(c_int, 1)) >> @as(c_int, 1)) > @as(c_int, 0)) {
    _ = &status;
    return (std.zig.c_translation.cast(i8, (status & @as(c_int, 0x7f)) + @as(c_int, 1)) >> @as(c_int, 1)) > @as(c_int, 0);
}
...
pub const __W_CONTINUED = std.zig.c_translation.promoteIntLiteral(c_int, 0xffff, .hex);
...
pub const RAND_MAX = std.zig.c_translation.promoteIntLiteral(c_int, 2147483647, .decimal);
...
pub const _SIGSET_NWORDS = std.zig.c_translation.MacroArithmetic.div(@as(c_int, 1024), @as(c_int, 8) * std.zig.c_translation.sizeof(c_ulong));
pub const __timeval_defined = @as(c_int, 1);
pub const _STRUCT_TIMESPEC = @as(c_int, 1);
pub const __suseconds_t_defined = "";
pub const __NFDBITS = @as(c_int, 8) * std.zig.c_translation.cast(c_int, std.zig.c_translation.sizeof(__fd_mask));
pub inline fn __FD_ELT(d: anytype) @TypeOf(std.zig.c_translation.MacroArithmetic.div(d, __NFDBITS)) {
    _ = &d;
    return std.zig.c_translation.MacroArithmetic.div(d, __NFDBITS);
}
pub inline fn __FD_MASK(d: anytype) __fd_mask {
    _ = &d;
    return std.zig.c_translation.cast(__fd_mask, @as(c_ulong, 1) << std.zig.c_translation.MacroArithmetic.rem(d, __NFDBITS));
}
...
pub const RAYGUI_ICON_DATA_ELEMENTS = std.zig.c_translation.MacroArithmetic.div(RAYGUI_ICON_SIZE * RAYGUI_ICON_SIZE, @as(c_int, 32));
...
pub inline fn TEXT_VALIGN_PIXEL_OFFSET(h: anytype) @TypeOf(std.zig.c_translation.MacroArithmetic.rem(std.zig.c_translation.cast(c_int, h), @as(c_int, 2))) {
    _ = &h;
    return std.zig.c_translation.MacroArithmetic.rem(std.zig.c_translation.cast(c_int, h), @as(c_int, 2));
}
